...
class TimeExclusionManager:
    ...
    def set_dataref_tracker(self, tracker):
        """
        Set the dataref tracker reference.
    ...
    def _get_config(self):
        ...
        Returns:
            tuple: (enabled, start_seconds, end_seconds) or (False, None, None) if disabled/invalid
        """
        try:
    ...
    def _check_exclusion_state(self):
        ...
        """
        enabled, start_sec, end_sec = self._get_config()
        ...
        if current_time < 0:
            # No valid sim time available, don't apply exclusion
            return False
