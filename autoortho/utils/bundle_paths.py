...
Bundle files are stored in a hierarchy matching X-Plane's scenery structure:
    {cache_dir}/bundles/{10°_band}/{1°_tile}/{row}_{col}_{maptype}.aob2

Example:
    cache/bundles/+30-130/+37-122/10880_10432_BI.aob2
"""
...
def get_bundle2_dir(cache_dir: str, row: int, col: int, zoom: int) -> str:
    """
...
def get_bundle2_filename(row: int, col: int, maptype: str) -> str:
    """
    Get the bundle filename (without directory).
    
    Format: {row}_{col}_{maptype}.aob2
    
    Note: Zoom level is NOT included in the filename since bundles
    can contain multiple zoom levels.
    ...
    Returns:
        Bundle filename like "10880_10432_BI.aob2"
    """
    return f"{row}_{col}_{maptype}.aob2"
...
def get_bundle2_path(cache_dir: str, row: int, col: int, 
    ...
    """
    bundle_dir = get_bundle2_dir(cache_dir, row, col, zoom)
    filename = get_bundle2_filename(row, col, maptype)
    return os.path.join(bundle_dir, filename)


def ensure_bundle2_dir(cache_dir: str, row: int, col: int, zoom: int) -> str:
    """
...
def parse_bundle_filename(filename: str) -> Optional[Tuple[int, int, str]]:
    """
    Parse a bundle filename to extract row, col, and maptype.
    
    Args:
        filename: Bundle filename (with or without .aob2 extension)
    
    Returns:
        Tuple of (row, col, maptype), or None if parsing fails
    """
